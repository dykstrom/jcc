...
public record WhileCodeGenerator(LlvmCodeGenerator cg, ConditionCodeGenerator ccg) implements LlvmStatementCodeGenerator<WhileStatement> {

    public interface ConditionCodeGenerator {
        LlvmOperand expression(final Expression expression, final List<Line> lines, final SymbolTable symbolTable);
    }
    ...
    public void toLlvm(final WhileStatement statement, final List<Line> lines, final SymbolTable symbolTable) {
        ...
        final var opCond = ccg.expression(statement.getExpression(), lines, symbolTable);
        lines.add(new BranchOperation(opCond, afterLabel, insideLabel));
