; E1 - baseline: a string argument produced in the frame that the become pops.
;
; Models the COL accumulator idiom, with the argument built from two string
; sub-expressions so that a registration (a collection point) sits between the
; loading of `acc` and its last use:
;
;   fun build(acc as string, n as i64) -> string :=
;       if n == 0 then acc else become build(concat(string(n), acc), n - 1)
;
; The IR shape is what jcc emits today: the COL become skeleton (tailcc,
; musttail immediately before ret, self-terminating tail-if leaves) carrying the
; GC plumbing the BASIC path emits (push/pop frame, rooted parameter, register
; then store into a rooted .gc.slot).
;
; Expected output: 123

source_filename = "e1-arg-across-pop"
...
L1:
  ; become build(concat(string(n), acc), n - 1)
  %5 = load i64, ptr %_n
  %6 = call ptr @col_string_i64(i64 %5)
  %7 = call ptr @jcc_gc_register(ptr %6)          ; may collect: acc must be rooted here
  store ptr %7, ptr %_.gc.slot.0
  %8 = load ptr, ptr %_acc                        ; acc used after that collection point
  %9 = call ptr @col_concat_str_str(ptr %7, ptr %8)
  %10 = call ptr @jcc_gc_register(ptr %9)         ; may collect: %7 is rooted in slot.0
  store ptr %10, ptr %_.gc.slot.1
  %11 = load i64, ptr %_n
  %12 = sub i64 %11, 1
  call void @jcc_gc_pop_frame()                   ; drops _acc, slot.0, slot.1
  %13 = musttail call tailcc ptr @build_Str_I64(ptr %10, i64 %12)
  ret ptr %13
}
...
entry:
  ...
  store ptr null, ptr %_.gc.slot.0
  call void @jcc_gc_add_root(ptr %_.gc.slot.0)
  %0 = call tailcc ptr @build_Str_I64(ptr @_.str.empty, i64 3)
