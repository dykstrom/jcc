; E3a - the mixed tail-if, exactly as ColFunDefCodeGenerator emits it today: the
; non-become leaf terminates its own block with a bare `ret`, because
; generateTail adds a ReturnOperation directly instead of going through
; ReturnCodeGenerator - so that leaf emits NO jcc_gc_pop_frame.
;
;   fun build(acc as string, n as i64) -> string :=
;       if n == 0 then acc else become build(concat(acc, "y"), n - 1)
;
; main calls build argv[1] times. Every call leaks one frame plus two roots that
; are never dropped, so the collector's frame and root arrays grow without bound:
; max RSS grows with the call count, while e3b - this file plus the one missing
; pop - stays flat. The leaked roots also make every later collection read
; `alloca` slots of native frames that have already returned.
;
; Expected output: yyyy

source_filename = "e3a-tail-if-no-pop"
...
L0:
  %4 = load ptr, ptr %_acc
  ; MISSING (as emitted today): call void @jcc_gc_pop_frame()
  ret ptr %4
