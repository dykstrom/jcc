; E2 - NEGATIVE CONTROL. Byte-for-byte E1 with one line removed: the callee no
; longer roots its string parameter (`call void @jcc_gc_add_root(ptr %_acc)`).
;
; `acc` is then reachable from nowhere - the producing frame was popped by the
; caller's become, and this frame never roots it - so the registration on the
; next line frees it, and the concat two lines later reads freed memory.
;
; This case MUST fail (an AddressSanitizer report, or wrong output). A clean run
; means the harness cannot see a use-after-free, and every other case's verdict
; is worthless.
;
; Expected: heap-use-after-free (ASan), and/or output that is not 123

source_filename = "e2-negative-control"

@_.printf.fmt.Str.nl = private constant [4 x i8] c"%s\0A\00"
@_.str.empty = private constant [1 x i8] c"\00"
@jcc.gc.global.roots = private global [1 x { ptr, i64 }] [{ ptr, i64 } { ptr null, i64 0 }]

declare ptr @col_concat_str_str(ptr, ptr)
declare ptr @col_string_i64(i64)
declare void @jcc_gc_add_root(ptr)
declare void @jcc_gc_init(i64, i64)
declare void @jcc_gc_pop_frame()
declare void @jcc_gc_push_frame()
declare ptr @jcc_gc_register(ptr)
declare void @jcc_gc_set_global_roots(ptr)
declare i32 @printf(ptr, ...)

; build(ptr, i64) -> ptr
define tailcc ptr @build_Str_I64(ptr %0, i64 %1) {
entry:
  call void @jcc_gc_push_frame()
  %_acc = alloca ptr
  store ptr %0, ptr %_acc
  %_n = alloca i64
  store i64 %1, ptr %_n
  %_.gc.slot.0 = alloca ptr
  %_.gc.slot.1 = alloca ptr
  ; MISSING (deliberately): call void @jcc_gc_add_root(ptr %_acc)
  store ptr null, ptr %_.gc.slot.0
  call void @jcc_gc_add_root(ptr %_.gc.slot.0)
  store ptr null, ptr %_.gc.slot.1
  call void @jcc_gc_add_root(ptr %_.gc.slot.1)
  %2 = load i64, ptr %_n
  %3 = icmp eq i64 %2, 0
  br i1 %3, label %L0, label %L1

L0:
  %4 = load ptr, ptr %_acc
  call void @jcc_gc_pop_frame()
  ret ptr %4

L1:
  ; become build(concat(string(n), acc), n - 1)
  %5 = load i64, ptr %_n
  %6 = call ptr @col_string_i64(i64 %5)
  %7 = call ptr @jcc_gc_register(ptr %6)          ; collects; acc is rooted nowhere
  store ptr %7, ptr %_.gc.slot.0
  %8 = load ptr, ptr %_acc                        ; dangling
  %9 = call ptr @col_concat_str_str(ptr %7, ptr %8)  ; reads freed memory
  %10 = call ptr @jcc_gc_register(ptr %9)
  store ptr %10, ptr %_.gc.slot.1
  %11 = load i64, ptr %_n
  %12 = sub i64 %11, 1
  call void @jcc_gc_pop_frame()
  %13 = musttail call tailcc ptr @build_Str_I64(ptr %10, i64 %12)
  ret ptr %13
}

define i32 @main() {
entry:
  call void @jcc_gc_init(i64 1, i64 0)
  call void @jcc_gc_set_global_roots(ptr @jcc.gc.global.roots)
  call void @jcc_gc_push_frame()
  %_.gc.slot.0 = alloca ptr
  store ptr null, ptr %_.gc.slot.0
  call void @jcc_gc_add_root(ptr %_.gc.slot.0)
  %0 = call tailcc ptr @build_Str_I64(ptr @_.str.empty, i64 3)
  store ptr %0, ptr %_.gc.slot.0
  %1 = call i32 (ptr, ...) @printf(ptr @_.printf.fmt.Str.nl, ptr %0)
  call void @jcc_gc_pop_frame()
  ret i32 0
}
