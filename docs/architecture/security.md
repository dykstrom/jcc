...
- JCC MUST NOT be relied on as a security boundary for compiling untrusted source. All command-line input — source, output path, `--clang`, `--library-path` — is assumed trusted; path validation and temp-file hardening are explicit non-goals. _Source: threat-model decision, 2026-06-03._
