...
Strings are COL's only heap type, and they are garbage collected: COL wires the same `RuntimeGcCodeGenerator` BASIC does, so a concatenation is registered with the collector and rooted, and `-print-gc`/`-initial-gc-threshold` work as they do for BASIC (see `docs/GarbageCollection.md` and `code-generation.md`). Nothing in the language exposes this — there is no free, no scope rule, and a string may be shared freely because strings are immutable. Concatenation allocates, with one exception: at `-O1` two string literals inside a statement the AST optimizer descends into are folded to a single literal (see `code-generation.md`).
