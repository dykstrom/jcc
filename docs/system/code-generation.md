...
The LLVM GC is being built in phases (issue #63; design in `docs/GarbageCollection.md` and ADR 0003). As of phase 3 the backend knows the `jcc_gc_*` runtime API (`JccGcBuiltIns` in `jcc-llvm`, constants prefixed `GF_`) and emits the shadow-stack plumbing: a `jcc_gc_push_frame`/`jcc_gc_pop_frame` frame around every function, `jcc_gc_add_root` for each string parameter and (null-initialized) string local, and — for `main` only — `jcc_gc_init(threshold, flags)` then `jcc_gc_set_global_roots` then the frame push, in that order, because `init` must be the first `jcc_gc_*` call. `threshold` is `-initial-gc-threshold`; `flags` carries the `JCC_GC_DEBUG` bit (1) when `-print-gc` is set. The `@jcc.gc.global.roots` table (`GcRootsOperation`) lists every non-constant string scalar (count 1) and string array (element count), null-terminated. Registration (`jcc_gc_register`) and removing the eager transient frees are phase 4, so no registration is emitted yet.

GC emission is gated by composition, not inheritance: a `GcCodeGenerator` strategy is injected into the shared `AbstractLlvmCodeGenerator`, `FunDefCodeGenerator`, and `ReturnCodeGenerator`. `NoOpGcCodeGenerator` (the default via the three-argument `AbstractLlvmCodeGenerator` constructor) emits nothing, so COL, Tiny, and Assembunny produce no `jcc_gc_*` calls; `BasicLlvmCodeGenerator` passes `RuntimeGcCodeGenerator`, which owns `main`'s init sequence and all frame/root emission. A language enables the collector by wiring in that strategy — no subclass, no flag (requirement 7: COL reuses the same strategy when it grows strings).
