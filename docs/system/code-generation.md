...
Inlining follows one pattern in both `BasicFunctions` and `ColFunctions`: two maps keyed by function identifier — a library map (built-in → library function) and an inline map (built-in → lambda building an AST expression from the call's arguments). `getInlineExpression` is a plain lookup. Its signature carries only the function and its arguments — no symbol table, no output lines — so an inline lowering that needs either is expressed as a dedicated AST node plus a code generator registered in the expression dictionary (`AscExpression`, `SgnExpression`, `LboundExpression`, `UboundExpression` in BASIC; `PrintlnExpression` in COL). New inlined built-ins should follow this shape rather than intercepting calls by name in a `FunctionCallExpression` code generator.

An inline builder must use each argument **exactly once**. What it returns goes straight to code generation, so an argument used twice is emitted twice: `SGN` written in the builder as `(x > 0) - (x < 0)` would make `SGN(RND)` draw two random numbers. A lowering that needs its argument more than once gets a dedicated AST node with one child, which its code generator evaluates once — that is what `SgnExpression` is for. Nothing enforces this, so the rule is stated on both `InlineBuilder` interfaces.

A library mapping must preserve the built-in's return type: `FunctionCallCodeGenerator` types the call result from the built-in, so mapping an I64-returning built-in to a double-returning intrinsic emits invalid IR that only Clang rejects — and only when a test exercises the function. When no library function has the right signature, use an inline mapping with a cast instead: `CINT` wraps `llvm.roundeven.f64` in a `CastToIntExpression`, while `INT` and `FIX` return a double (issue #62) and so map straight to `llvm.floor.f64` and `llvm.trunc.f64`.
...
Coverage is exactly the set of components registered in `BasicCodeGenerator` merged with the base dictionaries, including the nodes the shared `DefaultAstOptimizer` emits at `-O1` (`ShiftLeftExpression`) — a node the optimizer emits must be registered. `AssignCodeGenerator` takes a `Scope`: the base dictionary registers it with `NONE` (declared-variable languages), `BasicCodeGenerator` re-registers it with `GLOBAL`, because an assignment can be a variable's first use.

`IncStatement`/`DecStatement` and their generators exist for Assembunny's `inc`/`dec` only; the base dictionary registers them with `NONE` because `AssembunnyCodeGenerator` pre-declares every register as a global. `DefaultAstOptimizer` used to rewrite `a = a + 1` into them, and `a = a + 7` into now-deleted `Add/Sub/Mul/IDivAssignStatement` nodes; those rewrites emitted byte-identical IR to the plain assignment (LLVM IR has no compound assignment — both paths are load/op/store) and were removed. `DefaultAstExpressionOptimizer` still folds constants, and for strings that is a real optimization rather than a restatement: folding `"a" + "b"` removes an `add_Str_Str` call, a `jcc_gc_register` call and the result's shadow-stack slot, none of which `clang -O2` can eliminate on its own. The dividing line is what the operation lowers to — an IR instruction LLVM reasons about, or an opaque call into `libjccbas`/`libjcccol`. See ADR 0007.
...
Arrays are covered for integer/float/string elements, single- and multi-dimensional, with `OPTION BASE`, `LBOUND`/`UBOUND`, `SWAP` of elements, and arbitrary-expression subscripts (`BasicCompileAndRunArrayIT`). Each array is a private `[N x T]` global with a separate `[D x i64]` dimension-size metadata global, `LBOUND`/`UBOUND` are lowered inline (`LibJccBasBuiltIns` declares no constants for the `libjccbas` lbound, ubound and option_base symbols, so nothing can call them), and `OPTION BASE` is consumed at compile time. See `docs/Arrays.md` and ADR 0002.
