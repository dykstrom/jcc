...
- All four languages (BASIC, Tiny, Assembunny, COL) are fully supported. A few narrow BASIC gaps remain (e.g. `SLEEP` cannot be run by an IT, only compiled; `LINE INPUT;` `inhibitNewline` has no effect) — see `docs/system/standard-libraries.md`.
