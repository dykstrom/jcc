...
- Hand-running the compiler litters the working tree in two different places. With no `-o` the executable goes to the **current directory** as `a.out` (gitignored), not next to the source. `-S` instead writes `<source>.ll` *next to the source*, and additionally leaves a native `<basename>.s` in the current directory; neither is gitignored. Do not add a blanket `*.ll`/`*.asm` ignore — reference `.asm` files under `jcc-compiler/src/test/resources/` and spike `.ll` files under `docs/working-notes/` are tracked, so a blanket pattern would silently hide new ones. Compiling an example in place therefore leaves an untracked `.ll` beside it.
