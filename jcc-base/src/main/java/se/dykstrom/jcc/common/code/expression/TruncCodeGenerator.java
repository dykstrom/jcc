...
import se.dykstrom.jcc.common.assembly.instruction.floating.TruncateFloatRegToFloatReg;
...
public class TruncCodeGenerator extends AbstractExpressionCodeGenerator<TruncExpression, TypeManager, AsmCodeGenerator> {

    public TruncCodeGenerator(final AsmCodeGenerator codeGenerator) { super(codeGenerator); }

    @Override
    public List<Line> generate(final TruncExpression expression, final StorageLocation location) {
        return withCodeContainer(cc -> {
            ...
            cc.add(new TruncateFloatRegToFloatReg(register, register));
        });
    }
}
