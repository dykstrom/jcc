...
/**
 * Compile-and-run integration tests for COL strings: literals, escapes,
 * concatenation, equality, and string-typed vals. The collector these programs depend on is
 * exercised in [ColGarbageCollectionIT].
 */
@Tag("LLVM")
class ColCompileAndRunStringIT : AbstractIntegrationTests() {
    ...
    fun shouldPrintStringLiteral() {
        ...
        runAndAssertSuccess(listOf(), listOf("", "hello"))
    }
    ...
    fun shouldCompareStringsByContent() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileAndAssertSuccess(sourcePath, language = COL)
        runAndAssertSuccess(listOf(), listOf("true", "false", "false", "true"))
    }
    ...
    fun shouldConcatenateInIfExpression() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileAndAssertSuccess(sourcePath, language = COL)
        runAndAssertSuccess(listOf(), listOf("yes!"))
    }
    ...
    // A string val inside a while body is covered by ColGarbageCollectionIT, whose loop is the
