...
/**
 * Compile-and-run integration tests for the BASIC LLVM backend, focusing on user-defined functions.
 *
 * @author Johan Dykstrom
 */
@Tag("LLVM")
class BasicLlvmCompileAndRunUserFunctionsIT : AbstractIntegrationTests() {
    ...
    fun functionShouldCallOtherFunction() {
        ...
        val sourcePath = createSourceFile(source, BASIC)
        compileLlvmAndAssertSuccess(sourcePath, BASIC)
        runLlvmAndAssertSuccess(listOf(), listOf("9", "0"))
    }
    ...
    fun callFunctionWithMoreThanFourArgs() {
        ...
        val sourcePath = createSourceFile(source, BASIC)
        compileLlvmAndAssertSuccess(sourcePath, BASIC)
        runLlvmAndAssertSuccess(listOf(), listOf("12345", "1"))
    }
    ...
    fun assignResultOfFunctionCall() {
        ...
        val sourcePath = createSourceFile(source, BASIC)
        compileLlvmAndAssertSuccess(sourcePath, BASIC)
        runLlvmAndAssertSuccess(listOf(), listOf("fooFOO", "barBAR"))
    }
}
