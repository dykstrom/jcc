...
/**
 * Compile-and-run integration tests for COL, specifically for testing user-defined functions.
 *
 * @author Johan Dykstrom
 */
@Tag("LLVM")
class ColCompileAndRunUserFunctionsIT : AbstractIntegrationTests() {
    ...
    fun shouldCallUserDefinedFunctionWithUserDefinedFunctionArgReturnedFromOtherFunction() {
        ...
        compileAndAssertSuccess(sourceFile, language = COL)
        runAndAssertSuccess(listOf(), listOf("5"))
    }
    ...
    fun shouldPrintFunctionAddress() {
        ...
        val sourceFile = createSourceFile(source, COL)
        compileAndAssertSuccess(sourceFile, language = COL)
        runAndAssertSuccess(listOf(), listOf("0x"))
    }
