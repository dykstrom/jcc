...
/**
 * Compile-and-run integration tests for the COL string built-ins on the LLVM backend: `len`,
 ...
 *
 * The string type itself is covered by [ColLlvmCompileAndRunStringIT]; strings are LLVM-only, so
 * there is no FASM counterpart to either class.
 */
@Tag("LLVM")
class ColLlvmCompileAndRunStringBuiltInIT : AbstractIntegrationTests() {
    ...
    fun shouldEchoStdinUntilEof() {
        ...
        runLlvmAndAssertSuccess(listOf("first", "second", "third"), listOf("first", "second", "third"))
    }
    ...
    fun shouldEchoFinalLineWithoutTrailingNewline() {
        ...
        val sourcePath = createSourceFile(ECHO_FILTER, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccessWithRawInput("alpha\nbeta", listOf("alpha", "beta"))
    }
    ...
    fun shouldEchoNothingForEmptyInput() {
        ...
        val sourcePath = createSourceFile(ECHO_FILTER, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccessWithRawInput("", listOf())
    }
    ...
    fun shouldPassNonAsciiInputThroughByteExact() {
        val sourcePath = createSourceFile(ECHO_FILTER, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccessWithRawInput("höstlöv\nκαλημέρα\n😀", listOf("höstlöv", "καλημέρα", "😀"))
    }
    ...
    fun shouldCountBytesNotCharacters() {
        ...
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("5", "0", "9", "4"))
    }
    ...
    fun shouldClampSubstrAtBothEnds() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("", "", "", "", "he", "lo", "hello"))
    }
    ...
    fun shouldFindIndexOfNeedle() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("0", "2", "0", "-1", "-1"))
    }
    ...
    fun shouldComposeIndexofWithSubstr() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("", "value"))
    }
    ...
    fun shouldConcatenateBuiltInResults() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("world (5 bytes)", "at 2"))
    }
    ...
    fun shouldReadStdinIntoStringOperations() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf("abcdef", "xy", ""), listOf("6:abc", "2:xy", "0:"))
    }
    ...
    fun shouldWidenReturnValueOfUserFunction() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("17", "1.500000"))
    }
    ...
    fun shouldTreatSameTypeCastAsIdentity() {
        ...
        val sourcePath = createSourceFile(source, COL)
        compileLlvmAndAssertSuccess(sourcePath, language = COL)
        runLlvmAndAssertSuccess(listOf(), listOf("17", "1.500000", "3", "2.500000"))
    }
