...
import se.dykstrom.jcc.basic.ast.SystemStatement;
...
public class SystemCodeGenerator extends AbstractStatementCodeGenerator<SystemStatement, BasicTypeManager, BasicCodeGenerator> {
    ...
    public SystemCodeGenerator(final BasicCodeGenerator codeGenerator) {
        super(codeGenerator);
    }

    @Override
    public List<Line> generate(SystemStatement statement) {
        return withCodeContainer(cc -> cc.addAll(codeGenerator.functionCall(FUN_EXIT, getComment(statement), List.of(statement.getExpression()))));
    }
}
